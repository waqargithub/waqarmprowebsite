...
document.write('<Table summary="'+summarySent1+summarySent2+summarySent3+summarySent4+summarySent5+summarySent6+summarySent7+'" class="priceTable">');

// write the first row with headers, which lists the width threshholds in the price grid.

document.write('<tr>');
document.write('<th id="height" class="widthsHeading">Height\\Width</th>');
for (columnCount=0; columnCount < gridWidths.length; columnCount++)
  {
  document.write('<th id="'+gridWidths[columnCount]+'" class="widthsHeading">'+gridWidths[columnCount]+'</th>');
  }
document.write('</tr>');
for (rowCount=0; rowCount < gridHeights.length; rowCount++)
  {
  document.write('<tr>');
  document.write('<td headers="height" class="heightsHeading">'+gridHeights[rowCount]+'</td>');
  for (colCount=0; colCount < gridWidths.length; colCount++)
    {  
    priceGridIndex=(gridHeights.length-1)*rowCount+colCount;
    document.write('<td headers="'+gridWidths[colCount]+'" class="prices">$'+priceGrid[priceGridIndex].toFixed(2)+'</td>');
    } //inner for loop no. 2
  document.write('</tr>');
